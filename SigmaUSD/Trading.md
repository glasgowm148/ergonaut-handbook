---
title: SigmaUSD Trading
description: 
published: true
date: 2021-03-05T11:38:49.696Z
tags: 
editor: markdown
dateCreated: 2021-03-04T14:06:42.085Z
---

> **DYOR! Purpose of this group is to discuss SigmaUSD internals.**
> [Telegram Group](https://t.me/SigmaUSD)
{.is-info}

<iframe src="https://docs.google.com/spreadsheets/d/e/2PACX-1vQnBzsEOi0Kz5bD0K5gq9hzdDJSvmkD92EgMXZW0rAh6rDztJCwA0Rl2CMAieegrWNQdI3sDlTknPd4/pubhtml?widget=true&amp;headers=false"></iframe>

# Articles
- [introduction-to-ergos-sigmausd-stablecoin-risk-and-reward-mechanism](https://veriumfellow.medium.com/introduction-to-ergos-sigmausd-stablecoin-risk-and-reward-mechanism-18690b52d672)
- **[ERG price in USD](/SigmaUSD)**
- https://ergoplatform.org/en/blog/2021-02-05-building-ergo-how-the-ageusd-stablecoin-works/
- https://ergoplatform.org/en/blog/2021_02_26-sigmausd-released/
- https://www.cryptoglobe.com/latest/2021/03/sigmausd-ergos-community-deployed-stablecoin-is-live/

# Resources

- [Google Sheet for Calculations](https://docs.google.com/spreadsheets/d/1JlbNKfywLiXCCvRWS4Yf6zcKhIR8lPaUdWz_0RUTB94/edit#gid=1414078143)

Based on the excel, answers to most usual sigRSV breakeven questions are:

**Assuming other things stay at current (03/03/21) levels,**

- ERG price needs to go from 3.69$ to 4.2$  to make up the fees and start making more than by just holding ERG.

OR

- The contract needs to accumulate 35000 ERG in fees, to make up the sigRSV purchase/sell fees and start making more than by just holding ERG.

OR some combination like

- Price going up to 4$ and 12500 ERG collected as fees.
## API / Explorer
- **[Here you can see](https://api.ergoplatform.com/api/v1/boxes/unspent/byTokenId/027e094309e6a3055a7ddfe49238ac9d99c4fd1edaaea33d4c47f86becd1f08f?limit=100) the current values:** SigUSD in R4, SigRSV in R5
- [Reserve Wallet](https://explorer.ergoplatform.)

![sigmausd.jpg](/sigmausd.jpg)